...
assert("new history item has position/industry/jd fields", h3[0].position === "状态默认测试" && "industry" in h3[0] && typeof h3[0].jd === "string");
...
assert("footer shows v0.9", code3.includes("JD适配判断器 v0.9"));
...
assert("resume card present", code3.includes("resumeCard"));
...
// 四字段协议：aiAnalyze 发送 {resume, jd, redLines, dislikes}
assert("aiAnalyze is async function", /async function aiAnalyze/.test(code3));
assert("aiAnalyze posts to /analyze", code3.includes("AI_BACKEND_URL + \"/analyze\""));
assert("aiAnalyze body has 4 fields", code3.includes("redLines: profileRedLines(), dislikes: profileDislikes()"));
assert("aiAnalyze detects legacy report format", code3.includes("旧版报告格式"));
